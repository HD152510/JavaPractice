...
public class ActionManager {
	...
	private static Action getAction(String actionName) {
		Action action = null;
		if ("insert".equalsIgnoreCase(actionName)) action = new InsertAction();
		else if ("delete".equalsIgnoreCase(actionName)) action = new DeleteAction();
		else if ("update".equalsIgnoreCase(actionName)) action = new UpdateAction();
		return action;
	}
